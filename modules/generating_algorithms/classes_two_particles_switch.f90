...
private
    ...
    type, extends(Abstract_Generating_Algorithm), abstract, public :: Abstract_Two_Particles_Switch
    private
    ...
    subroutine Abstract_visit_short(this, overlap, delta_energies, ij_actors, new, old)
        ...
        do i = 1, size(new)
            do i_component = 1, size(this%short_interactions%visitable_cells, 1)
                ...
                call this%short_interactions%visitable_cells(i_component, ij_actors(i))%&
                    visit_energy(overlap, energies_new(i_component, i), new(i), visit_different, &
                    i_exclude)
                if (overlap) return
            end do
        end do
        do i = 1, size(old)
            do i_component = 1, size(this%short_interactions%visitable_cells, 1)
                ...
                call this%short_interactions%visitable_cells(i_component, ij_actors(i))%&
                    visit_energy(overlap, energies_old(i_component, i), old(i), visit_different, &
                    i_exclude)
            end do
        end do
        delta_energies = energies_new - energies_old
