...
private
public :: create_identities, create_positions

contains

    !> @todo Handle Null case
    subroutine create_identities(particles_transmutation, physical_model, changes)
        class(Abstract_Generating_Algorithm), allocatable, intent(out) :: &
            particles_transmutation
        ...
        if (count(can_exchange(:, 1)) > 1) then
            allocate(Box_Particles_Swap_Identities :: particles_transmutation)
        else
        ...
        select type (particles_transmutation)
            ...
            class default
                call error_exit("procedures_box_particles_swap_factory: create_identities: "//&
    ...
    subroutine create_positions(particles_switch, physical_model, changes)
        ...
        if (count(can_translate(:, 1)) > 1) then
            allocate(Box_Particles_Swap_Positions :: particles_switch)
        else
        ...
        select type (particles_switch)
            ...
            type is (Null_Generating_Algorithm)
            class default
                call error_exit("procedures_box_particles_swap_factory: create_positions: "//&
