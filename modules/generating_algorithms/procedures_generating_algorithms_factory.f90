...
contains
    ...
    subroutine create(generating_algorithms, physical_model, changes)
        ...
        if (size(generating_algorithms) /= 9) then
        ...
        call box_particle_translation_create(generating_algorithms(4)%algorithm, &
            physical_model, changes%components)
        call box_particle_rotation_create(generating_algorithms(5)%algorithm, &
            physical_model, changes%components)
        call box_particle_add_create(generating_algorithms(6)%algorithm, physical_model, &
            changes)
        call box_particle_remove_create(generating_algorithms(7)%algorithm, physical_model, &
            changes)
        call box_particles_transmutation_create(generating_algorithms(8)%algorithm, &
            physical_model, changes)
        call box_particles_switch_create(generating_algorithms(9)%algorithm, &
            physical_model, changes)
