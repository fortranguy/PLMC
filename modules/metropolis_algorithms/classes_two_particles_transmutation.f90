...
!implementation Abstract_Two_Particles_Transmutation
    ...
    subroutine Abstract_try(this, observables)
        ...
        if (success) then
            ...
            observables%transmutations_counters(ij_actors(1), ij_actors(2))%num_success = &
                observables%transmutations_counters(ij_actors(1), ij_actors(2))%num_success + 1
        end if
        ...
        do i = size(particles), 1, -1
            do i_component = 1, size(this%short_interactions%visitable_cells, 1)
                ...
                call this%short_interactions%visitable_cells(i_component, ij_actors(i))%&
                    visit(overlap, deltas(i_component, i), particles(i), i_exclude)
                if (overlap) return
            end do
        end do
        deltas(:, 1) = -deltas(:, 1)
