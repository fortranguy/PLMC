    ...
    \begin{tikzpicture}
        ...
        \begin{scope}[shift={($(ChangeTuner -| ComponentExchange.north) - (0 cm, 0.5 cm)$)}]
            \begin{class}[text width=\smallTextWidth]{ChangeTuningParameters}{0, 0}
                \attribute{increaseFactor\type{double}}
                \attribute{increaseFactorMax\type{double}}
            \end{class}
        \end{scope}

        \begin{scope}[shift={($(ChangeTuner.north -| ChangeTuningParameters.north)$)}]
