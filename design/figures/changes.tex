...
\begin{document}
    ...
    \begin{tikzpicture}[scale=\scale, every node/.style={scale=\scale}]
        ...
        \begin{scope}[shift={($(MoveTuner.north) + (8.5 cm, -1.5 cm)$)}]
        ...
        \begin{scope}[shift={($(CoordinatesCopier.north) + (0 cm, 4 cm)$)}]
            \begin{class}[text width=\mediumTextWidth]{Changes}{0, 0}
            \attribute{randomPosition: \type{RandomCoordinates}}
            \attribute{randomOrientation: \type{RandomCoordinates}}
            \attribute{positionCopier: \type{CoordinatesCopier}}
            \attribute{orientationCopier: \type{CoordinatesCopier}}
            \attribute{components: \type{ChangesComponent[:]}}
            \end{class}
        \end{scope}
