\documentclass{standalone}

\input figures/figures.sty

\begin{document}

    \begin{tikzpicture}

        \begin{package}{Mixture}

            \begin{scope}
                \begin{class}[text width=\mediumTextWidth]{Component}{0, 0}
                    \attribute{number\type{ComponentNumber}}
                    \attribute{positions\type{ComponentCoordinates}}
                    \attribute{orientations\type{ComponentCoordinates}}
                    \attribute{chemicalPotential\type{ComponentChemicalPotential}}
                    \attribute{dipolarMoments\type{ComponentDipolarMoments}}
                \end{class}
            \end{scope}

            \begin{scope}[shift={($(Component.south) - (0 cm, 1.5 cm)$)}]
                \begin{abstractclass}[text width=\smallTextWidth]{MinimumDistance}{0, 0}
                    \operation{set(minDistance\type{double})}
                    \operation{get()\type{double}}
                \end{abstractclass}
            \end{scope}

            \begin{scope}[shift={($(Component.north) + (0 cm, 7 cm)$)}]
                \begin{abstractclass}[text width=\mediumTextWidth]{MixtureTotalMoment}{0, 0}
                    \operation{construct(components\type{Component[:]}, areDipolar\type{bool[:]})}
                    \operation{destroy()}
                    \operation{reset()}
                    \operation{isDipolar(iComponent\type{int})}
                    \operation{get()}
                    \operation{add(iComponent\type{int}, dipolarMoment\type{double[3]})}
                    \operation{remove(iComponent\type{int}, dipolarMoment\type{double[3]})}
                \end{abstractclass}
            \end{scope}

        \end{package}

        \begin{scope}[on background layer]
            \aggregation{MixtureTotalMoment}{components}{1..n}{Component}
        \end{scope}

    \end{tikzpicture}

\end{document}
