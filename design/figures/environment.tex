...
\begin{document}
    ...
    \begin{tikzpicture}[scale=\scale, every node/.style={scale=\scale}]
        ...
        \begin{scope}[shift={($(BoxSizeChecker |- BetaPressureExcess.north)$)}]
            \begin{abstractclass}[text width=\largeTextWidth]{BoxVolumeMemento}{0, 0}
                \operation{\procedure{construct}{periodicBox: \type{PeriodicBox}}}
                \operation{\procedure{destroy}{}}
                \operation{\procedure{save}{}}
                \operation{\procedure{getRatio}{}: \type{real}}
            \end{abstractclass}
        \end{scope}
