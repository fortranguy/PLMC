...
implicit none
    ...
    call boxes_create(periodic_boxes, generating_data, environment_prefix)
    ...
    max_distance = minval(periodic_boxes(1)%get_size()) / 2._DP
    ...
    do i_snap = 1, num_snaps
        ...
        do i_particle = 1, component_1%num_particles
            do j_particle = 1, component_2%num_particles
                distance_ij = periodic_boxes(1)%distance(component_1%positions(:, i_particle), &
    ...
    component_2%density = real(component_2%num_particles_sum, DP) / real(num_snaps) / &
        product(periodic_boxes(1)%get_size())
