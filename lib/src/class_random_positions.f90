...
private
    ...
    private
        class(Abstract_Box_Geometry), pointer :: box_geometry => null()
...
!implementation Abstract_Random_Positions
    ...
    function Abstract_Random_Positions_move(this, i_sphere) result(position)
        ...
        position = this%dipolar_spheres%get_position(i_sphere)        
...
!implementation Bulk_Random_Positions
    ...
    function Bulk_Random_Positions_position(this) result(position)
        ...
        call random_number(random_vector)
        position = this%box_geometry%get_size() * random_vector        
...
!implementation Slab_Random_Positions
    ...
    function Slab_Random_Positions_position(this) result(position)
        ...
        call random_number(random_vector)
        position(1:2) = reshape(this%box_geometry%get_size(), [2]) * random_vector(1:2)
        position(3) = (this%box_geometry%get_height() - this%dipolar_spheres%get_diameter()) * &
                      random_vector(3) + this%dipolar_spheres%get_diameter() / 2._DP
