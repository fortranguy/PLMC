...
private
    ...
    contains
        procedure :: set => Null_Minimum_Distance_set
        procedure :: get => Null_Minimum_Distance_get
...
!implementation Null_Minimum_Distance
    ...
    subroutine Null_Minimum_Distance_set(this, min_distance)
        class(Null_Minimum_Distance), intent(inout) :: this
        real(DP), intent(in) :: min_distance
    end subroutine Null_Minimum_Distance_set

    pure function Null_Minimum_Distance_get(this) result(min_distance)
        class(Null_Minimum_Distance), intent(in) :: this
        real(DP) :: min_distance
        min_distance = 0._DP
    end function Null_Minimum_Distance_get
