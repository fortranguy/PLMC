\begin{package}{Environment}
    ...
    \begin{scope}[shift={($(PeriodicBox.north) + (-11 cm, 6.5 cm)$)}, opacity=1]
        \begin{abstractclass}[text width=\mediumTextWidth]{ParallelepipedDomain}{0, 0}
            ...
            \operation{overlap(parallelepipedDomain\type{ParallelepipedDomain})}
    ...
    \begin{scope}[shift={($(ExternalField.north) + (10 cm, 0 cm)$)}, opacity=1]
        \begin{abstractclass}[text width=\mediumTextWidth]{WallsPotential}{0, 0}
            \operation{construct(gap\type{double}, floorPenetration\type{FloorPenetration},
                pairPotential\type{PairPotential})}
            \operation{destroy()}
            \operation{getGap()}
            \operation{meet(overlap\type{bool}, energy\type{double}, position\type{double[3]})}
        \end{abstractclass}
    \end{scope}
