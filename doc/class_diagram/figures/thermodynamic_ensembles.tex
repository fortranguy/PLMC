\begin{scope}[shift={($(Box.north) + (-5 cm, 85 cm)$)}, opacity=0.25]
    \begin{package}{Thermodynamic ensembles}
        ...
        \begin{scope}[opacity=1]
            \begin{abstractclass}[text width=\largeTextWidth]{OneParticleMove}{0, 0}
                ...
                \operation{setCandidateObservables(moveCounters\type{ChangeCounter[2]},
                    particlesEnergies\type{ParticleEnergy[2]}, interEnergy\type{double})}
                \operation{destroy()}
                \operation{try()}
            \end{abstractclass}
        \end{scope}
