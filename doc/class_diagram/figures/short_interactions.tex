\begin{scope}[shift={($(Box.north) + (-28 cm, 55 cm)$)}, opacity=0.5]
\begin{package}{Short range interactions}
    ...
    \begin{scope}[opacity=1]
        \begin{abstractclass}[text width=\mediumTextWidth]{ParticlesPotential}{0, 0}
            ...
            \operation{destroy()}
            \operation{set(positions\type{ParticlesPositions})}
    ...
    \begin{scope}[shift={($(ParticlesPotential.north) + (-12 cm, 0 cm)$)}, opacity=1]
        \begin{abstractclass}[text width=\largeTextWidth]{VisitableCells}{0, 0}
            \operation{construct(mold\type{VisitableList}, periodicBox\type{PeriodicBox},
                                    positions\type{ParticlesPositions}, pairPotential\type{PairPotential})}
            \operation{destroy()}
            \operation{fill()}
            \operation{visit(particle\type{Particle}, overlap\type{bool},
                energy\type{double})}
    ...
    \begin{scope}[shift={($(ParticlesPotential.south) + (0 cm, -2 cm)$)}, opacity=1]
        \begin{abstractclass}[text width=\mediumTextWidth]{PairPotential}{0, 0}
            ...
            \operation{meet(distance\type{double}, overlap\type{bool}, energy\type{double})}
        \end{abstractclass}
    \end{scope}
    ...
    \begin{scope}[shift={($(PairPotential.north) + (10 cm, 0 cm)$)}, opacity=1]
        \begin{abstractclass}[text width=\mediumTextWidth]{WallPotential}{0, 0}
            \operation{construct(potentialDomain\type{PotentialDomain},
                potentialExpression\type{PotentialExpression})}
            \operation{destroy()}
            \operation{meet(distance\type{double}, overlap\type{bool}, energy\type{double})}
...
\begin{scope}[on background layer]
    ...
    \aggregation{WallPotential}{potentialExpression}{1}{PotentialExpression}
\end{scope}
