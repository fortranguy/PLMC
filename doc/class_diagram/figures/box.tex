\begin{package}{Box}
    ...
    \begin{scope}[shift={($(AbstractExternalField.north) + (10 cm, 0 cm)$)}, opacity=1]
        \begin{abstractclass}[text width=\mediumTextWidth]{WallsPotential}{0, 0}
            \operation{construct(gap\type{double}, potentialDomain\type{PotentialDomain},
                potentialExpression\type{PotentialExpression})}
            \operation{destroy()}
            \operation{getGap()}
            \operation{meet(position\type{double[3]}, overlap\type{bool}, energy\type{double})}
        \end{abstractclass}
    \end{scope}
